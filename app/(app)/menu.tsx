...
export default function MenuScreen() {
  ...
  const addItem = async () => {
    ...
    const id = await createOrder(itemsArray);
    if (id) {
      setOrderId(id);
      setQuantities({});
    }
      ...
      <ScrollView style={{ maxHeight: 300 }}>
        ...
        {Object.entries(quantities).some(([_, qty]) => qty > 0) && (
          <>
            ...
            <Text style={{ fontWeight: 'bold', marginBottom: 5 }}>📝 Orden actual (no enviada):</Text>
        ...
        {pedidos.length > 0 ? (
          pedidos.map((pedido, index) => (
            <View key={pedido.id} style={{ marginBottom: 10 }}>
              <Text style={{ fontWeight: 'bold' }}>🧾 Pedido #{index + 1}</Text>
